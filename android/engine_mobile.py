...
class MobileGlowEngine:
    ...
    def _task_blueprints(self, day):
        ...
        return [
            {"id": 1, "name": "Morning Water", "target_time": "04:55", "note": "Drink 400-500ml before jogging.", "requires_proof": True},
            {"id": 2, "name": "30-Min Morning Jog/Walk", "target_time": "05:00", "note": "Outdoor cardio. Weather can switch this to indoor.", "requires_proof": True},
            {"id": 3, "name": "GlutaGenC (Pre-Breakfast)", "target_time": "06:00", "note": "Take before breakfast then wait 30-60 min.", "requires_proof": True},
            {"id": 4, "name": "Small Breakfast", "target_time": "06:45", "note": "Keep calories controlled and low-rice.", "requires_proof": True},
            {"id": 5, "name": "Probiotic Supplement", "target_time": "07:05", "note": "Take shortly after breakfast.", "requires_proof": True},
            {"id": 6, "name": "Afternoon Abs/Fat-Loss Workout", "target_time": "16:15", "note": " | ".join(exercises), "requires_proof": True},
            {"id": 7, "name": "GlutaGenC (Bedtime)", "target_time": "20:30", "note": "Take before final wind-down.", "requires_proof": True},
            {"id": 8, "name": "Skin Care Routine", "target_time": "20:40", "note": "Night routine before sleep lock starts.", "requires_proof": True},
            {"id": 9, "name": "Sleep Reminder / Lockout", "target_time": "21:00", "note": "Sleep window enforced until 05:00.", "requires_proof": True},
        ]
    ...
    def _normalize_tasks(self, raw_tasks, day):
        ...
        for task in raw_tasks:
            if isinstance(task, dict) and isinstance(task.get("id"), int):
                by_id[task["id"]] = task
        return self._build_tasks(day=day, existing_by_id=by_id)
    ...
    def load_data(self):
        ...
        weather = loaded.get("weather", defaults["weather"])
    ...
    def _advance_day_if_needed(self):
        today = date.today()
        raw = self.data.get("last_reset_date", today.isoformat())
        try:
        ...
        next_day = min(30, current_day + max(0, delta))
    ...
    def refresh_daily_plan(self):
        ...
        self.data["tasks"] = self._build_tasks(day=day, existing_by_id=existing)
    ...
    def count_completed(self):
        return sum(1 for task in self.get_tasks() if task.get("done"))

    def is_morning_ready(self):
        morning = [task for task in self.get_tasks() if task.get("id") in {1, 2}]
        if not morning:
            return False
